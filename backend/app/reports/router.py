...
async def get_report_details(
    ...
    measurements = db.query(ReportMeasurement).filter(ReportMeasurement.report_id == report.id).all()
...
async def delete_report(
    ...
    db.query(ReportMeasurement).filter(ReportMeasurement.report_id == report.id).delete()
    db.delete(report)
    db.commit()
...
async def analyze_report(
    ...
    if not report.raw_text:
        raise HTTPException(status_code=400, detail="Report has no text to extract. OCR required.")
...
async def summarize_report(
    ...
    measurements = db.query(ReportMeasurement).filter(ReportMeasurement.report_id == report.id).all()
    if not measurements:
        raise HTTPException(status_code=400, detail="No extracted measurements found. Run analysis first.")
    ...
    if summary.get("status") == "verification_failed":
        raise HTTPException(status_code=500, detail="Summary generation failed hallucination verification.")
