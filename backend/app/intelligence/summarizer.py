...
def verify_summary_against_evidence(summary: Dict[str, Any], evidence: Dict[str, Any]) -> bool:
    """
    ...
    for finding in key_findings:
        ...
        if biomarker not in allowed_biomarkers:
            logger.error(f"Hallucination detected: Biomarker '{biomarker}' not in evidence.")
            return False
        ...
        if ev.get("severity") != allowed.get("severity"):
            logger.error(f"Hallucination detected: Severity mismatch for '{biomarker}'.")
            return False
...
def mock_llm_call(evidence: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "normal_findings": [f"{normal_count} biomarkers were within their reported reference ranges."],
        "follow_up_considerations": ["Discuss these findings with your healthcare professional."],
